...
// DripGroundBoardParser parses venue data from drip_ground_board source
type DripGroundBoardParser struct {
	mongodb *mongo.Database
}

// NewDripGroundBoardParser creates a new parser for drip_ground_board
func NewDripGroundBoardParser(mongodb *mongo.Database) *DripGroundBoardParser {
	return &DripGroundBoardParser{
		mongodb: mongodb,
	}
}
...
// For drip_ground_board, we query the ground_board_periods collection
func (p *DripGroundBoardParser) Parse(taskID string, rawData map[string]interface{}) ([]types.VenueSnapshot, error) {
	...
	// Build query filter - note: drip_ground_board also uses tennis_booking project_id
	filter := bson.M{
