...
var (
	...
	scriptBase = flag.String("scripts", "", "Lua scripts base path (e.g., /app/scripts or ./crawler/scripts)")
)
...
func main() {
	...
	viper.SetDefault("crawler.script_base", "crawler/scripts")
	...
	if mode == "enhanced" {
		...
		if err := mongoDB.Connect(connectCtx); err != nil {
			...
			// Get script base path (command line overrides config, then env, then default)
			scriptBasePath := viper.GetString("crawler.script_base")
			if *scriptBase != "" {
				scriptBasePath = *scriptBase
			} else if envScriptBase := os.Getenv("CRAWLER_SCRIPT_BASE"); envScriptBase != "" {
				scriptBasePath = envScriptBase
			}

			logger.Info("Lua script configuration",
				"script_base", scriptBasePath,
				"source", func() string {
					if *scriptBase != "" {
						return "command-line"
					} else if os.Getenv("CRAWLER_SCRIPT_BASE") != "" {
						return "environment"
					} else if viper.IsSet("crawler.script_base") {
						return "config-file"
					}
					return "default"
				}())
			...
			enhancedConfig := &distributed.EnhancedNodeConfig{
				...
				MongoDBWrapper: mongoDB,        // Pass the wrapper too
				ScriptBase:     scriptBasePath, // Pass script base path
			}
